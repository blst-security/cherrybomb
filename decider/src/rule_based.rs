...
where T:Clone+Eq+PartialEq+Default+std::hash::Hash{
...
fn test_payload(cur:&String,other:&PayloadDescriptor)->u16{
    ...
    for param in params{
        if let Some(index) = other.params.iter().position(|p| p.name.trim()==param.param.trim()){
    ...
    for ep in session.req_res.clone(){
        ...
        if let Some(i) = eps_path.iter().position(|p| p== &(&ep.path)){
            ...
            if ep.status != 404{
                ...
                anomaly_score+=test_payload(&ep.res_payload,&ep_digest.req_res_payloads.res_payload);
            }else{
                anomaly_score+=1;
            }
        }else if !cond1{
