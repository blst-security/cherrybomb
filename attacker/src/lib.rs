...
impl Attacker{
    pub fn save(&self)->Result<(),std::io::Error>{
        let mut file = OpenOptions::new().write(true).truncate(true).create(true).open(FILE)?;
...
pub fn get_populations()->Vec<Vec<String>>{
    let attacker = if let Ok(a) = Attacker::load(){ a } else { return vec![]; };
...
pub fn refit(pop:usize,anomalies:Vec<Option<Anomaly>>,anomaly_scores:Vec<Vec<u16>>){
    let mut attacker = if let Ok(a) = Attacker::load(){ a } else { return; };
    attacker.populations[pop].refit(anomalies,anomaly_scores);
    attacker.save().unwrap();
}
pub async fn attack(pop:usize,verbosity:Verbosity,decide_file:&str,headers:&[Header],auth:&Authorization)->Result<Vec<Session>,&'static str>{
    if let Ok(mut file) = OpenOptions::new().write(true).create(true).open(decide_file){
        if let Ok(attacker) = Attacker::load(){ 
            let sessions = attacker.populations[pop].run_gen(verbosity,&attacker.base_url,headers,auth).await;
