...
fn params_to_payload(ep:&str,params:Vec<Parameter>)->(String,String,String,Vec<Header>){
    ...
    for param in params{
        match param.dm{
            ...
            QuePay::Headers=>{
                headers.push(Header{name:param.name,value:param.value});
            }
            _=>(),
        }
    }
...
async fn send_attack(base_url:&str,eps:Vec<(Method,String,Vec<Parameter>)>,headers:&[Header],auth:&Authorization)->Vec<ReqRes>{
    let mut rr = vec![];
    for ep in eps{
...
pub async fn attack_flow(base_url:&str,genes:&[Gene],headers:&[Header],auth:&Authorization)->(Vec<ReqRes>,Vec<String>){
    let mut eps = vec![];
    let mut choises =vec![];
    for gene in genes{
