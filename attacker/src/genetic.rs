...
impl Population{
    pub fn new(group:&Group,max_members:u16,min_members:u16,top_score:Option<u16>,mutation_rate:u8,cross_rate:u8)->Population{
        ...
        let genomes = (0..min_members).map(|_i| Genome::new(&group.endpoints)).collect::<Vec<Genome>>();
    ...
    pub fn endpoints(&self)->Vec<String>{ self.endpoints.clone() }
    fn print(&self,verbosity:Verbosity){
        match verbosity{
            Verbosity::Verbose=>{
                let cs = self.genomes[0].genes().iter().map(|g| g.chromosomes().len() as u8).collect::<Vec<u8>>();
                let fs = self.top_genomes.iter().map(|g| g.fitness()).collect::<Vec<u16>>();
                let print = format!("group:{:?}
generation:{}
population size:{}
genes in genomes:{}
...
fixation level:{}
",self.endpoints,self.generation,self.genomes.len(),self.genomes[0].genes().len(),cs,self.fitness_score,fs,self.mutations_acc,self.fixation);
                println!("{}",print);
            },
            Verbosity::Default=>{
                let cs = self.genomes[0].genes().iter().map(|g| g.chromosomes().len() as u8).collect::<Vec<u8>>();
...
number of chromosomes for each gene:{:?}
fitness score:{}
",self.endpoints,self.generation,self.genomes.len(),self.genomes[0].genes().len(),cs,self.fitness_score);
                println!("{}",print);
            },
            Verbosity::Basic=>{
                let print = format!("group:{:?}
...
population size:{}
fitness score:{}
",self.endpoints,self.generation,self.genomes.len(),self.fitness_score);
                println!("{}",print);
            },
            Verbosity::None=>{
                let print = format!("group:{:?}
...
",self.endpoints,self.generation,self.fitness_score);
    ...
    pub async fn run_gen(&self,verbosity:Verbosity,base_url:&str,headers:&[Header],auth:&Authorization)->Vec<Session>{
    ...
    fn update_gene_pool(&mut self){
        for genome in self.genomes.iter(){
            genome.genes().iter().for_each(|gene| {self.gene_pool.insert(gene.clone());});
        }
    }
