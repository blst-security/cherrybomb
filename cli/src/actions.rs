...
fn parse_map_file(digest:Digest) -> Result<String, serde_json::Error> {
    let new_groups = digest.groups.iter().map(|group| {
        WebMapGroup {
            endpoints: group.endpoints.iter().map(|ep| ep.path.path_ext.clone()).collect::<Vec<String>>(),
            links: group.links.iter().map(|link| WebMapLink{from:link.from.path.path_ext.clone(), to:link.to.path.path_ext.clone(), strength:link.strength}).collect::<Vec<WebMapLink>>(),
        }
...
fn write_map_file(file_name:String, map:String) {
    match OpenOptions::new().write(true).create(true).open(format!("{}.json", &file_name)) {
...
pub fn add_token(token:String) -> bool {
    match Uuid::parse_str(&token) {
        Ok(_) => {
            match OpenOptions::new().write(true).create(true).open(format!("{}.txt", "token")) {
...
pub fn map(logs_file:String, output:String) {
    ...
    if !sessions.is_empty() {
        ...
        write_map_file(output.clone(), parse_map_file(digest).unwrap_or_else(|_| {print_err("Failed parsing digest into web map"); String::new()}));
...
pub fn prepare_attacker(mut url:String, map_file:String) {
    let d_map:Digest = match read_file(&format!("{}_checkpoint.json", map_file)){
        Some(s_map) => {
            match serde_json::from_str(&s_map) {
                Ok(r) => r,
                Err(e) => {
        ...
        None => {
            print_err(&format!("Failed reading map file \"{}_checkpoint\"", &map_file));
            return;
        }
    };
    match Url::parse(&url) {
...
pub async fn attack_domain(map_file:String, decide_file:String, pop:usize, gen:usize, verbosity:Verbosity, headers:Vec<Header>, auth:Authorization) {
    let d_map:Digest = match read_file(&format!("{}_checkpoint.json", map_file)){
        Some(s_map) => {
            match serde_json::from_str(&s_map) {
                Ok(r) => r,
    ...
    for _ in 0..gen {
        ...
        match attack(pop, verbosity, &format!("{}.json", decide_file),&headers,&auth).await {
            Ok(vec_sessions) => {
...
pub fn load(logs_file:String, map_file:String) {
    ...
    if !sessions.is_empty() {
        ...
        write_map_file(map_file.clone(), parse_map_file(d_map).unwrap_or_else(|_| {print_err("Failed parsing digest into web map"); String::new()}));
