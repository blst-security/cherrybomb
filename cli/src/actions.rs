...
pub fn map(logs_file:String, output:String) {
    ...
    if !sessions.is_empty() {
        ...
        write_map_file(format!("{}", output), match serde_json::to_string(&digest) {
            Ok(r) => r,
            Err(_) => {
                print_err(&format!("Failed writing to \"{}\" file", output));
                return;
            }
        });
...
pub fn prepare_attacker(mut url:String, map_file:String) {
    let d_map:Digest = match read_file(&format!("{}.json", map_file)){
        Some(s_map) => {
        ...
        None => {
            print_err(&format!("Failed reading map file \"{}\"", &map_file));
            return;
        }
    };
    match Url::parse(&url) {
...
pub fn load(logs_file:String, map_file:String) {
    ...
    let mut d_map:Digest = match read_file(&format!("{}.json", map_file)){
        Some(s_map) => {
            match serde_json::from_str(&s_map) {
                Ok(r) => r,
                Err(e) => {
    ...
    if !sessions.is_empty() {
        ...
        write_map_file(format!("{}", map_file), match serde_json::to_string(&d_map) {
            Ok(r) => r,
            Err(_) => {
                print_err(&format!("Failed writing to \"{}\" file", map_file));
                return;
            }
        });
        println!("{}", "Your map is ready, you can upload and view it at https://www.blstsecurity.com/firecracker/Visualizer".green());
