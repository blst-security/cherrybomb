...
use swagger::{Swagger,OAS3_1,OAS,Check,ParamTable,EpTable, Authorization, PassiveScanType};
...
where T:OAS+Serialize+for<'de> Deserialize<'de>+std::fmt::Debug{
    ...
    let failed = scan.passive_checks.iter().any(|c| 
         c.result()=="FAILED"); //TODO support minimum level failure
...
where T:OAS+Serialize+for<'de> Deserialize<'de>+std::fmt::Debug{
    ...
    scan.print(verbosity);
    let failed = scan.checks.iter().any(|c|(c.result()=="FAILED") || (c.top_severity() >Level::Info));
    if let Some(f) = output_file{
        let print = scan.print_to_file_string();
        write_to_file(&f,print);
    }
    if failed { Ok(101) }  else { Ok(0) }
}

pub async fn run_swagger(
    file:&str,verbosity:u8,
    output_file:Option<String>,
    auth: Authorization,
    ...
    if version.starts_with("3."){ 
        ...
        let active_result = run_active_swagger_scan::<OAS3_1>(
            ...
            verbosity,output_file.clone(),
            auth,
            active_scan_type).await;
