...
impl ParamValue{
    pub fn from(schema:&Schema)->Self{
        let v = if let Some(t) = schema.schema_type.clone() { t } else { String::new() };
...
impl Param{
    ...
    fn object_to_param(swagger:&Value,schema:Schema,p_type:String,required:bool)->Param{
        ...
        let s1 = schema.clone();
        let requireds = if let Some(r) = schema.required{ r }else{ vec![] };
        if let Some(items) = schema.items{
            let inner = items.inner(swagger); 
            let r = Self::required(&inner,&p_type,requireds.clone());
            params.push(Param::change_from(Self::schema_rec(swagger,inner,r),String::from("items")));
        }
        if let Some(all) = schema.all_of{
            for sc in all{
                params.push(Param::change_from(Self::schema_rec(swagger,sc.inner(swagger),true),String::from("all")));
            }
        }
        if let Some(any) = schema.any_of{
            for sc in any{
                let r = Self::required(&sc.inner(swagger),&p_type,requireds.clone());
                params.push(Param::change_from(Self::schema_rec(swagger,sc.inner(swagger),r),String::from("any")));
            }
        }
        if let Some(one) = schema.one_of{
            for sc in one{
                let r = Self::required(&sc.inner(swagger),&p_type,requireds.clone());
                params.push(Param::change_from(Self::schema_rec(swagger,sc.inner(swagger),r),String::from("one")));
            }
        }
        if let Some(not) = schema.not{
            let r = Self::required(&s1,&p_type,requireds.clone());
            params.push(Param::change_from(Self::schema_rec(swagger,not.inner(swagger),r),String::from("not")));
        }
        if let Some(prop) = schema.properties{
            for (p_name,p) in prop{
                let r = Self::required(&p.inner(swagger),&p_type,requireds.clone());
                params.push(Param::change_prop(p_name,Self::schema_rec(swagger,p.inner(swagger),r),String::from("prop")));
            }
        }
        let name = if let Some(n) = schema.title { n } else { String::new() };
        let description = if let Some(d) = schema.description { d } else { String::new() };
    ...
    pub fn schema_rec(swagger:&Value,schema:Schema,required:bool)->Self{
        let p_type = if let Some(t) = schema.schema_type.clone() { t } else { String::new() };
        match p_type.to_lowercase().as_str(){
            "array"|""|"object"=>Self::object_to_param(swagger,schema,p_type,required),
            "number"|"boolean"|"integer"|"string"=>Self::simple_to_param(schema,String::from("string"),required),
            _=>{println!("{:?}",p_type); Param::default()},
        }
    }
    pub fn schema_to_params(swagger:&Value,schema:SchemaRef,name:String,required:bool)->Self{
        let mut params:Vec<Param> = vec![];
        let mut schemas:Vec<Schema> = vec![];
        let sc = schema.inner(swagger);
        if let Some(all) = sc.all_of{
            schemas.extend(all.iter().map(|s| s.inner(swagger)).collect::<Vec<Schema>>());
        }
        if let Some(any) = sc.any_of{
            schemas.extend(any.iter().map(|s| s.inner(swagger)).collect::<Vec<Schema>>());
        }
        if let Some(one) = sc.one_of{
            schemas.extend(one.iter().map(|s| s.inner(swagger)).collect::<Vec<Schema>>());
        }
        ...
        for schema in schemas{
            let p_type = if let Some(t) = schema.schema_type.clone() { t } else { String::new() };
            let requireds = if let Some(r) = schema.required.clone(){ r }else{ vec![] };
