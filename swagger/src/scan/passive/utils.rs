...
pub fn check_servers_for_server_url_rule(servers:&[Server],location:&str,prev_addrs:&mut HashSet<String>)->Vec<Alert>{
    let mut alerts= vec![]; 
    for server in servers{
        ...
        match Url::parse(&server.url){
            Ok(u)=>{
                if u.scheme()=="http"{
                    alerts.push(Alert::new(Level::Low,"Insecure transport, using http instead of https",format!("{}, address:{}",location,server.url)));
                }
            },
            Err(ParseError::RelativeUrlWithoutBase)=>continue,
            Err(_)=>alerts.push(Alert::new(Level::Low,"Invalid URL in the server parameter",format!("{}, address:{}",location,server.url))),
        }; 
    }
    alerts
}
    ...
    for (path,item) in &swagger.paths{
        ...
        for (method,operation) in item.get_ops(){
            params.extend(operation.params().iter().map(|param| {
                let param = param.inner(swagger_value);
                (Param::schema_to_params(swagger_value,param.schema(),param.name(),param.required()),format!("swagger path:{} method:{} param:{}",path,method,param.name()))
            }));
            let (req_payload_type,req_payload_params) = operation.req_body(swagger_value);
            params.extend(req_payload_params.iter().map(|param| (param.clone(),format!("swagger path:{} request body, payload type:{:?}",path,req_payload_type))));
    ...
    match &param.value{
        ...
        ParamValue::Integer(i)=>{
            if let Some(sc) = &i.default{
                match sc{
                    SchemaStrInt::Int(_)=>{return vec![];},
                    _=>{
                        return vec![Alert::new(Level::Low,"Default type does not match parameter type",format!("{} param name:{}",loc,param.name))]; 
                    }
                }
            }
        },
...
pub fn additional_properties_test(schema:&Schema,location:String)->Vec<Alert>{
    let tp = if let Some(t) = &schema.schema_type { t } else { "" };
    let mut alerts = vec![];
        ...
        for (m,op) in item.get_ops(){
            if let Some(r_body) = &op.request_body{
                ...
                for (name,m_t) in r_body.content{
                    if let Some(s) = m_t.schema{
                        schemas.extend(get_all_params_by_type(&s.inner(swagger_value),swagger_value,"integer",format!("swagger rooot path:{} method:{} request body media type:{}",path,m,name))); 
                    }
                }
            }
...
pub fn get_all_params_by_type(schema:&Schema,swagger_value:&Value,tp:&str,location:String)->Vec<(Schema,String)>{
    let mut schemas = vec![]; 
    let s_tp = if let Some(t) = &schema.schema_type { t } else { "" }; 
    if s_tp==tp{
        schemas.push((schema.clone(),location.clone()));
    }
    match s_tp.to_lowercase().as_str(){
        "object"|""|"array"=>{
            let any_of = if let Some(a) = &schema.any_of { a.to_vec() } else { vec![] };
            let one_of = if let Some(a) = &schema.one_of { a.to_vec() } else { vec![] };
            let all_of = if let Some(a) = &schema.all_of { a.to_vec() } else { vec![] };
            let props = if let Some(p) = &schema.properties { p.clone() } else { HashMap::new() };
            schemas.extend(any_of.iter().map(|a| get_all_params_by_type(&a.inner(swagger_value),swagger_value,tp,location.clone())).flatten());
            schemas.extend(all_of.iter().map(|a| get_all_params_by_type(&a.inner(swagger_value),swagger_value,tp,location.clone())).flatten());
            schemas.extend(one_of.iter().map(|a| get_all_params_by_type(&a.inner(swagger_value),swagger_value,tp,location.clone())).flatten());
            schemas.extend(props.iter().map(|(name,p)| get_all_params_by_type(&p.inner(swagger_value),swagger_value,tp,format!("{} prop:{}",location,name))).flatten());
        },
        _=>(),
    };
    schemas
