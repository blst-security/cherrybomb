    ...
    fn check_int_attrs(&self)->Vec<Alert>{
        ...
        for (schema,location) in schemas{
            if schema.minimum.is_none(){
                alerts.push(Alert::new(Level::Low,"Number schema without a minimum",location.clone()));
            }
            if schema.maximum.is_none(){
                alerts.push(Alert::new(Level::Low,"Number schema without a maximum",location));
            }
        }
    ...
    fn check_str_attrs(&self)->Vec<Alert>{
        ...
        for (schema,location) in schemas{
            if schema.min_length.is_none(){
                alerts.push(Alert::new(Level::Low,"String schema without a minimum length",location.clone()));
            }
            if schema.max_length.is_none(){
                alerts.push(Alert::new(Level::Low,"String schema without a maximum length",location.clone()));
            }
            if schema.pattern.is_none(){
                alerts.push(Alert::new(Level::Low,"String schema without a pattern",location));
            }
        }
        alerts
    }
    fn check_arr_attrs(&self)->Vec<Alert>{
        ...
        for (schema,location) in schemas{
            if schema.min_items.is_none(){
                alerts.push(Alert::new(Level::Info,"Array schema without an item minimum",location.clone()));
            }
            if schema.max_items.is_none(){
                alerts.push(Alert::new(Level::Low,"Array schema without an item maximum",location));
            }
        }
        alerts
    }
    fn check_obj_attrs(&self)->Vec<Alert>{
        ...
        for (schema,location) in schemas{
            if schema.min_properties.is_none(){
                alerts.push(Alert::new(Level::Low,"Object schema without minimum properties",location.clone()));
            }
            if schema.max_properties.is_none(){
                alerts.push(Alert::new(Level::Low,"Object schema without maximum properties",location.clone()));
            }
